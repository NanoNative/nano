...
`EVENT_CONFIG_CHANGE` is the central hook for updating running services without restarting Nano. Any component can broadcast a partial config map, and services merge the new values immediately. When the [FileWatcher](../services/filewatcher/README.md) is active it publishes those events automatically for watched files, which means HTTP/TLS certificates, application profiles, or custom config overlays can be rotated by touching the files on disk. Services such as `HttpServer` combine both mechanisms: they listen for `EVENT_CONFIG_CHANGE` to apply new paths and rely on FileWatcher-driven `EVENT_FILE_CHANGE` notifications to re-read existing paths. Ensure FileWatcher runs alongside services that expect hot reloads; otherwise you must emit `EVENT_CONFIG_CHANGE` manually.
