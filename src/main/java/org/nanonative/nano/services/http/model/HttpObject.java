...
public class HttpObject extends HttpRequest {
    ...
    public TypeInfo<?> bodyAsJson() {
        ...
        final Object type = body.startsWith("<") ? XmlDecoder.xmlTypeOf(body) : body.isEmpty() ? "" : JsonDecoder.jsonOf(body);
        return type instanceof TypeInfo<?> result ? result : new TypeList().addR(type);
    }
    ...
    @Override
    public URI uri() {
        final String pathPart = path() == null ? "" : path();
        final TypeMap queryParams = queryParams();
        ...
        final StringBuilder uriBuilder = new StringBuilder(pathPart);
        uriBuilder.append("?");

        boolean first = true;
        for (final Object keyObj : queryParams.keySet()) {
            final String key = keyObj.toString();
            if (!first) {
                uriBuilder.append("&");
            }
            final String value = queryParams.asString(key);
            uriBuilder.append(key).append("=").append(value != null ? value : "");
            first = false;
        }
